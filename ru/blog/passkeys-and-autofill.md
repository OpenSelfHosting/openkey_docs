...
В приложении включите OpenKey в **Settings → Autofill** как системного провайдера паролей и passkeys. Этот путь покрывает приложения и браузеры, которые обращаются к хранилищу учётных данных ОС — дополняет расширение, но не заменяет его на каждой платформе.
